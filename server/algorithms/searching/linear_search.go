...
func NewLinearSearch() *LinearSearch {
	return &LinearSearch{
		BaseAlgorithm: algorithms.BaseAlgorithm{
			...
			Description:     "线性搜索是最简单的搜索算法。它依次检查数组中的每个元素，直到找到目标元素或检查完所有元素。",
			TimeComplexity:  "O(n)",
			SpaceComplexity: "O(1)",
...
func (ls *LinearSearch) Search(data []interface{}, target interface{}, tracker models.StepTracker) (int, error) {
	...
	tracker.AddNote("将逐个检查每个元素")

	// 逐个检查数组中的每个元素
	for i := 0; i < n; i++ {
		tracker.SetPhase("搜索第 " + strconv.Itoa(i+1) + " 个元素")
		
		// 高亮当前检查的元素
		tracker.AddStep("检查位置 "+strconv.Itoa(i)+": "+ls.toString(data[i]), data, []int{i})
		...
		if cmp == 0 {
			...
			// 元素不匹配，继续搜索
			tracker.AddNote("元素不匹配，继续搜索")
		}
	}

	// 未找到目标元素
	tracker.SetPhase("完成")
	tracker.AddStep("检查完所有元素，未找到目标", data, []int{})
	tracker.AddNote("搜索失败，共检查了 " + strconv.Itoa(n) + " 个元素")
	return -1, nil
}
